...
def make_relay_driver_class(extension_set: str = "none"):
    ...
    The extension *specs* (names, types, docs) come from the canonical
    vpcam_extensions metadata; the accessors are bound to CA relay calls.
    """
    from vpcam_extensions import (COMMON_EXTENSION_SPECS,
                                  IMX708_EXTENSION_SPECS)

    if extension_set == "none":
        specs = []
    elif extension_set == "imx708":
        specs = COMMON_EXTENSION_SPECS + IMX708_EXTENSION_SPECS
    else:
        specs = list(COMMON_EXTENSION_SPECS)
